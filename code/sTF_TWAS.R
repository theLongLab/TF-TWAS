  	...
  	if (ncol(cis_gt) >= 2) {
			...
			print("elnet_out")
				...
				print(eval$weighted_snps)
        			...
        			weighted_snps_info <- snp_annot %>% filter(chrpos %in% eval$weighted_snps) %>% select(SNP,chrpos,ref,effect)
				...
				if (nrow(weighted_snps_info) != 0){
        				...
        				do_covariance(gene, gt_cov_df, weighted_snps_info$SNP, weighted_snps_info$chrpos, covariance_file)
